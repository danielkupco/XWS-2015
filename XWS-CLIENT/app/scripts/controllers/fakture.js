'use strict';
 ...
 angular.module('fakture', ['resource.faktura',
 	'angular-md5'])

 .controller('faktureCtrl', function (Invoice, $scope, $location, md5, $log) {
...
*/
	$scope.fakture = Faktura.query();
	$log.info($scope.fakture.length);//0
	//kada smo kliknuli na red u tabeli prelazimo na stranicu za editovanje fakture sa zadatim id-om
 	$scope.insertOrEditInvoice = function (faktura) {
 		if(faktura){
 			$location.path('/fakture/'+faktura.id);
 		}
 		else{
			$location.path('/fakture/new');
 		}
 	}
 });
