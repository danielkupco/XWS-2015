...
public class CentralnaBankaImpl implements CentralnaBanka {
    ...
    public xws.tim7.entities.globals.StatusType primiMT102(xws.tim7.entities.mt102.MT102Type nalogZaGrupnaPlacanja) { 
        ...
        try {
        	...
        	Banka_BankaPort_Client tmpClient = new Banka_BankaPort_Client(idBankePoverioca);
        	tmpClient.primiMT102(nalogZaGrupnaPlacanja);
        	
        	// prosledi mt900/910 bankama koje ce proslediti novac firmama
            for(NalogZaPlacanjeType nzp : nalogZaGrupnaPlacanja.getNalogZaPlacanje()) {
            	if(nzp.getOsnovaNalogaZaPlacanje().getIznos().compareTo(new BigDecimal(250000)) < 0) {
	            	xws.tim7.entities.globals.ObjectFactory factory = new xws.tim7.entities.globals.ObjectFactory();
	                
	            	Banka_BankaPort_Client clientK = new Banka_BankaPort_Client(nzp.getOsnovaNalogaZaPlacanje().getRacunDuznika().getBrojRacuna());
	                MT9XXType mt900 = factory.createMT900Type(nzp);
	                clientK.primiMT900(mt900);
	                
	                Banka_BankaPort_Client clientD = new Banka_BankaPort_Client(nzp.getOsnovaNalogaZaPlacanje().getRacunPoverioca().getBrojRacuna());
	                MT9XXType mt910 = factory.createMT910Type(nzp);
